...
def render(d):
    p=d['palette']; bg=p.get('background','#f4efe8'); paper=p.get('paper','#fffaf2')
    ink=p.get('ink','#292421'); accent=p.get('accent','#b34f71'); sage=p.get('secondary','#75836d')
    sections=''.join(section_html(s) for s in d['sections'])
    cta=d.get('cta') or {}; cta_html=''
    if cta.get('label') and cta.get('href'):
        cta_html=f'<a class="cta" href="{esc(cta["href"])}">{esc(cta["label"])} ↗</a>'
