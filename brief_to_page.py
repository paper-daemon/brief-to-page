...
def load(path):
    ...
    data.setdefault('palette',{}); data.setdefault('lang','ja')
    ...
    if not isinstance(data['lang'], str) or not LANG.fullmatch(data['lang']):
        raise ValueError('lang must be a simple BCP 47 language tag such as ja, en, or en-US')
...
def render(d):
    ...
    canonical=safe_meta_url(d.get('canonical'),'canonical')
    og_image=safe_meta_url(d.get('og_image'),'og_image')
    ...
    og_image_html=f'<meta property="og:image" content="{esc(og_image)}">' if og_image else ''
    lang=esc(d.get('lang','ja'))
    return f'''<!doctype html><html lang="{lang}"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{esc(d['title'])}</title><meta name="description" content="{esc(d['description'])}"><meta property="og:title" content="{esc(d['title'])}"><meta property="og:description" content="{esc(d['description'])}">{canonical_html}{og_image_html}
...
def main():
    ...
    ap.add_argument('--validate-only',action='store_true',help='validate the full brief, palette and URLs without writing HTML')
    a=ap.parse_args()
    try:
        data=validate_brief(a.brief)
        if a.validate_only:
            print(f'valid=1 sections={len(data["sections"])} brand={data["brand"]}')
            return
